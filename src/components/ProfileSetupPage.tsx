...
const inputCls = `
  w-full px-3 py-3 rounded-2xl text-center text-[#3B2A16] placeholder-[#A89167]
  bg-[#E9DAB8] border border-[#D8C290] text-sm font-medium
  focus:outline-none focus:border-[#9A6A12] focus:ring-2 focus:ring-[#9A6A1220] transition
`.trim()
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="px-6 pt-14 pb-8 text-center">
        ...
        <div className="flex justify-center mb-5">
          <div className="relative">
            <div className="absolute inset-0 rounded-full bg-[#9A6A12]/10 blur-xl" />
            <div className="relative w-[64px] h-[64px] rounded-full p-[2.5px]"
              ...
              <div className="w-full h-full rounded-full bg-[#FBF4E2] flex items-center justify-center">
                <span className="text-3xl" style={{ color: '#9A6A12', fontFamily: 'serif' }}>☯</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-2.5 mb-3">
          {user.picture ? (
            ...
            <div className="w-8 h-8 rounded-full bg-[#9A6A1220] border border-[#9A6A1240] flex items-center justify-center">
              <span className="text-[#9A6A12] text-sm font-bold">{user.name[0]}</span>
            </div>
          )}
          <p className="text-[#5C4A2E] text-sm font-semibold">{user.name}님, 반가워요</p>
        </div>
        <h1 className="text-xl font-bold text-[#3B2A16] mb-1" style={{ fontFamily: "'Gowun Batang', serif" }}>
          내 정보 입력
        </h1>
        <p className="text-[#9A8155] text-xs leading-relaxed">
      ...
      <div className="flex-1 px-4 -mt-4">
        <form
          ...
          className="bg-[#FBF4E2] rounded-3xl shadow-[0_4px_24px_rgba(201,150,42,0.10)] border border-[#D8C290] p-6 space-y-5"
        >
          {/* 프로필 사진 */}
          <div>
            <p className="text-xs font-semibold text-[#9A6A12] mb-2 flex items-center gap-1.5">
              <span className="w-1 h-4 bg-[#9A6A12] rounded-full inline-block" />
              프로필 사진 <span className="text-[#9A8155] font-normal">(선택)</span>
            </p>
            <div className="flex items-center gap-3">
              <div className="relative shrink-0">
                {photo ? (
                  <img src={photo} alt="프로필 사진" className="w-16 h-16 rounded-full object-cover border border-[#9A6A1240]" />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-[#E9DAB8] border border-[#D8C290] flex items-center justify-center text-2xl">🙂</div>
                )}
              </div>
              <div className="flex-1 space-y-1.5">
                ...
                <button
                  ...
                  className="w-full py-2 rounded-xl text-xs font-semibold bg-[#E9DAB8] border border-[#D8C290] text-[#5C4A2E] hover:border-[#9A6A1240] transition"
                >
                  {photo ? '사진 변경' : '사진 선택'}
                </button>
                {photo && (
                  <button
                    ...
                    className="w-full py-1.5 rounded-xl text-[11px] font-semibold text-[#9A8155] hover:text-rose-400 transition"
                  >
                    삭제
                  </button>
                )}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-[#9A6A12] flex items-center gap-1.5">
                <span className="w-1 h-4 bg-[#9A6A12] rounded-full inline-block" />
                생년월일
              </p>
              <div className="flex gap-1 bg-[#E9DAB8] border border-[#D8C290] rounded-full p-0.5">
                {(['solar', 'lunar'] as const).map(c => (
                  <button
                    key={c} type="button"
                    onClick={() => setCalendarType(c)}
                    className={`px-3 py-1 rounded-full text-[11px] font-semibold transition ${
                      calendarType === c ? 'bg-[#9A6A12] text-[#0D0A1A]' : 'text-[#9A8155]'
                    }`}
                  >
                    {c === 'solar' ? '양력' : '음력'}
                  </button>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={f.label}>
                  <label className="block text-[11px] text-[#6E5836] mb-1.5 font-medium text-center">{f.label}</label>
                  <input
            ...
            {calendarType === 'lunar' && (
              <label className="flex items-center gap-1.5 text-xs text-[#6E5836] cursor-pointer select-none mt-2.5">
                <input
                  type="checkbox" checked={isLeapMonth}
                  onChange={e => setIsLeapMonth(e.target.checked)}
                  className="accent-[#9A6A12] rounded w-3.5 h-3.5"
                />
                윤달이에요
              </label>
            )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[11px] text-[#6E5836] mb-1.5 font-medium text-center">시</label>
                <input
              ...
              <div>
                <label className="block text-[11px] text-[#6E5836] mb-1.5 font-medium text-center">분</label>
                <input
          ...
          {/* 성별 */}
          <div>
            <p className="text-xs font-semibold text-[#9A6A12] mb-3 flex items-center gap-1.5">
              <span className="w-1 h-4 bg-[#9A6A12] rounded-full inline-block" />
              성별
            </p>
            <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl font-bold text-[#0D0A1A] text-base
              ...
              shadow-lg shadow-[#9A6A1230] transition-all active:scale-[0.98]"
          >
            운명봄 시작하기 →
          </button>
        </form>

        <p className="text-center text-xs text-[#A89167] my-5">
