...
const EMOJI: Record<string, string>  = { wood: '🌳', fire: '🔥', earth: '🪨', metal: '⚔️', water: '💧' }
  ...
  return (
    <div className="bg-white rounded-3xl border border-stone-100 shadow-[0_2px_16px_rgba(124,58,237,0.07)] p-6">
      <div className="flex items-center gap-2 mb-5">
        ...
        <h2 className="text-base font-bold text-stone-800" style={{ fontFamily: "'Noto Serif KR', serif" }}>오행 분포 (五行)</h2>
      </div>
        ...
        {elements.map(el => {
          ...
          return (
            <div key={el} className="flex items-center gap-3">
              <span className="text-base w-5">{EMOJI[el]}</span>
              <span className="w-14 text-sm text-stone-500 font-medium">{ELEMENT_LABELS[el]}</span>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        <div className="rounded-2xl p-3 border" style={{ backgroundColor: ELEMENT_COLORS[strongest] + '10', borderColor: ELEMENT_COLORS[strongest] + '30' }}>
          ...
          <p className="font-bold text-sm" style={{ color: ELEMENT_COLORS[strongest] }}>{EMOJI[strongest]} {ELEMENT_LABELS[strongest]}</p>
          <p className="text-xs text-stone-400 mt-0.5">{TRAITS[strongest]}</p>
        </div>
        {count[weakest] < count[strongest] && (
          <div className="rounded-2xl p-3 border border-stone-200 bg-stone-50">
            <p className="text-xs text-stone-400 mb-1">약한 기운 (보완)</p>
            <p className="font-bold text-sm" style={{ color: ELEMENT_COLORS[weakest] }}>{EMOJI[weakest]} {ELEMENT_LABELS[weakest]}</p>
            <p className="text-xs text-stone-400 mt-0.5">{TRAITS[weakest]}</p>
          </div>
        )}
      </div>
    </div>
