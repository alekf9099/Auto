      ...
      <div className="max-w-2xl mx-auto px-4 py-5">
        ...
        {step === 'form' && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-[#FBF4E2] via-[#F3E7C8] to-[#EAD9B0] rounded-3xl p-5 border border-[#E0528225] shadow-xl shadow-[#000]/40">
                  ...
                  {(['male','female'] as const).map(g => (
                    <button
                      ...
                      className={`py-2.5 rounded-xl text-sm font-semibold border transition-all ${
                        birth.gender === g
                          ? 'bg-[#E0528220] border-[#E05282] text-[#3B2A16]'
                          : 'bg-[#E9DAB8] border-[#D8C290] text-[#9A8155] hover:border-[#E0528250]'
                      }`}
                    >
                      {g === 'male' ? '남성 🧑' : '여성 👩'}
                    </button>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['year','month','day'] as const).map(f => (
                    <input
                      ...
                      className="bg-[#E9DAB8] border border-[#D8C290] rounded-xl px-3 py-2.5 text-sm text-[#3B2A16] placeholder:text-[#A89167] focus:outline-none focus:border-[#E05282] transition text-center"
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="0~23시"
                    min={0} max={23}
                    className="w-full bg-[#E9DAB8] border border-[#D8C290] rounded-xl px-3 py-2.5 text-sm text-[#3B2A16] placeholder:text-[#A89167] focus:outline-none focus:border-[#E05282] transition text-center"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    min={0} max={59}
                    disabled={birth.hour === ''}
                    className="w-full bg-[#E9DAB8] border border-[#D8C290] rounded-xl px-3 py-2.5 text-sm text-[#3B2A16] placeholder:text-[#A89167] focus:outline-none focus:border-[#E05282] transition text-center disabled:opacity-40"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 justify-center">
              ...
              <p className="text-[10px] text-[#A89167]">사주팔자 일간 오행 기반</p>
              <div className="h-px flex-1 bg-[#D8C290]"/>
            </div>
          </form>
        )}
        ...
        {step === 'loading' && (
          <div className="flex flex-col items-center justify-center py-24 space-y-6">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-[#9A8155]">사주 오행을 분석해 스타일을 추천합니다</p>
            </div>
          </div>
        )}
        ...
        {step === 'result' && result && (
          <div className="space-y-4 animate-fade-in-up">
            ...
            <div className="rounded-3xl p-6 border shadow-xl shadow-[#000]/40"
              style={{ background: 'linear-gradient(135deg, #FBF4E2 0%, #F3E7C8 60%, #EAD9B0 100%)', borderColor: 'rgba(224,82,130,0.35)' }}
            >
              ...
              <p className="text-xs text-[#9A8155] mb-4">{result.todayData.mood}</p>
                    ...
                    return (
                      ...
                      >
                        <div className="relative">
                          <div
                            className={`rounded-xl shadow-md transition-transform hover:scale-110 ${c.main ? 'w-12 h-12' : 'w-9 h-9'} ${active ? 'ring-2 ring-[#E05282] ring-offset-2 ring-offset-[#FBF4E2] scale-110' : 'ring-1 ring-white/10'}`}
                        ...
                        <p className={`text-[9px] text-center max-w-[48px] leading-tight ${active ? 'text-[#3B2A16] font-semibold' : 'text-[#9A8155]'}`}>{c.name}</p>
                      </button>
                    )
                  })}
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-center gap-3 bg-[#0000002A] border border-[#FFFFFF14] rounded-2xl px-4 py-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[#3B2A16]">{shown.name}</p>
                        <p className="text-xs text-[#9A8155] uppercase">{shown.hex}</p>
                      </div>
                    </div>
                  )
                })()}
              ...
              <div className="flex gap-2 items-start bg-[#E052820D] border border-[#E0528230] rounded-2xl px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#5C4A2E] leading-relaxed">
                    당신의 일간은 <span className="text-[#3B2A16] font-bold">{result.myData.elementChi}({result.myData.element})</span> 기운이에요.
            ...
            <div className="bg-[#FBF4E2] rounded-3xl border border-[#D8C290] p-5 shadow-[0_2px_20px_rgba(122,90,40,0.15)]">
              ...
              <div className="space-y-3">
                {ITEM_LABELS.map(({ key, label, emoji }) => (
                  <div key={key}>
                    <p className="text-[10px] text-[#9A8155] font-semibold mb-1.5">{emoji} {label}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {result.todayData.items[key].map((item, i) => (
                        <span
                          key={item}
                          className={i === 0
                            ? "text-xs font-bold bg-[#E0528220] border border-[#E05282] text-[#3B2A16] pl-2.5 pr-3 py-1 rounded-full inline-flex items-center gap-1"
              ...
              <div className="flex gap-3">
                {result.todayData.avoidColors.map(c => (
                  <div key={c.hex} className="flex items-center gap-2">
                    ...
                    <p className="text-xs text-[#9A8155]">{c.name}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#FBF4E2] via-[#F3E7C8] to-[#EAD9B0] rounded-3xl p-5 border border-[#9A6A1225] shadow-xl shadow-[#000]/40">
              <div className="flex gap-2 items-start bg-[#9A6A120D] border border-[#9A6A1230] rounded-2xl px-4 py-3">
                <span className="text-sm flex-shrink-0">💬</span>
                <div>
                  <p className="text-[10px] text-[#9A6A12] font-bold mb-0.5">스타일링 팁</p>
                  <p className="text-sm text-[#5C4A2E] leading-relaxed">{result.todayData.tip}</p>
                </div>
              </div>
            </div>
