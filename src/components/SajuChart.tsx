...
function PillarCell({ label, stemIndex, branchIndex }: { label: string; stemIndex: number; branchIndex: number }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <span className="text-xs text-stone-400 mb-2 font-medium">{label}</span>

      {/* 천간 */}
      <div
        className="w-16 h-16 rounded-2xl flex flex-col items-center justify-center border-2 mb-1 shadow-sm"
        style={{ borderColor: stemColor, backgroundColor: stemColor + '15' }}
      >
        <span className="text-2xl font-bold" style={{ color: stemColor }}>{stem.hanja}</span>
      ...
      {/* 지지 */}
      <div
        className="w-16 h-20 rounded-2xl flex flex-col items-center justify-center border-2 shadow-sm"
        style={{ borderColor: branchColor, backgroundColor: branchColor + '15' }}
      >
        <span className="text-2xl font-bold" style={{ color: branchColor }}>{branch.hanja}</span>
        <span className="text-xs font-medium" style={{ color: branchColor }}>{branch.ko}</span>
        <span className="text-xs mt-0.5" style={{ color: branchColor + 'aa' }}>{branch.animal}</span>
      </div>
