...
function PillarCell({ label, stemIndex, branchIndex, compact = false }: {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="text-xs mb-2 text-stone-400">
    ...
    <div className="bg-white rounded-3xl border border-stone-100 shadow-[0_2px_16px_rgba(124,58,237,0.07)] p-6">
      ...
      <div className="mt-5 pt-4 border-t border-stone-100">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-xs bg-violet-50 text-violet-600 border border-violet-200 px-2 py-0.5 rounded-full font-medium">일주</span>
          <span className="text-sm font-semibold text-stone-800">
            {STEMS[dayPillar.stemIndex].hanja}{BRANCHES[dayPillar.branchIndex].hanja}
            <span className="text-stone-400 font-normal text-xs ml-1">({STEMS[dayPillar.stemIndex].ko}{BRANCHES[dayPillar.branchIndex].ko}일)</span>
          </span>
        </div>
        <p className="text-sm text-stone-500 leading-relaxed">
          일간 <strong className="text-stone-700">{STEMS[dayPillar.stemIndex].hanja}({STEMS[dayPillar.stemIndex].ko})</strong>이 본명(本命)입니다.{' '}
