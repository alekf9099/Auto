  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} className={`text-base ${i < n ? 'text-[#B5841C]' : 'text-[#D8C290]'}`}>★</span>
      ))}
    </div>
  )
}
        ...
        <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size} className="absolute inset-0">
          <defs>
            <linearGradient id="needleGradient" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <stop offset="100%" stopColor="#B5841C" />
            </linearGradient>
            <linearGradient id="arcGradient" x1="50%" y1="0%" x2="50%" y2="100%">
              <stop offset="0%" stopColor="#B5841C" />
          ...
          <circle cx={cx} cy={cy} r="6" fill="#E9DAB8" stroke="url(#needleGradient)" strokeWidth="1.5" />
        </svg>
        <div className="absolute inset-x-0 flex flex-col items-center" style={{ top: cy + 28 }}>
          <p className="text-2xl font-bold text-[#B5841C]" style={{ fontFamily: "'Gowun Batang', serif", textShadow: '0 1px 6px rgba(0,0,0,0.7)' }}>{percent}%</p>
          <p className="text-xs font-semibold text-[#4A3820]" style={{ textShadow: '0 1px 6px rgba(0,0,0,0.7)' }}>{luckLabel(percent)}</p>
...
function CategoryRow({ icon, label, text, star }: { icon: React.ReactNode; label: string; text: string; star: number }) {
  ...
  return (
    <div className="border border-[#9A6A1220] rounded-2xl overflow-hidden">
      <button
        className="w-full flex items-center gap-2.5 p-3.5 text-left hover:bg-[#E9DAB8] transition"
        onClick={() => setOpen(o => !o)}
        aria-expanded={open}
        aria-label={`${label} ${open ? '접기' : '펼치기'}`}
      >
        <GoldBadge><span className="text-[#9A6A12]">{icon}</span></GoldBadge>
        <span className="text-sm font-semibold text-[#4A3820]">{label}</span>
        <span className="text-xs text-[#9A8155] flex-1 truncate">{text.slice(0, 18)}…</span>
        <Stars n={star} />
        <span className="text-[#A89167] text-xs ml-1">{open ? '▲' : '▼'}</span>
      </button>
      {open && (
        <div className="px-4 pb-4">
          <div className="h-px bg-[#9A6A1220] mb-3" />
          <p className="text-sm text-[#6E5836] leading-relaxed">{text}</p>
        </div>
      )}
    </div>
  )
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-[#FBF4E2] border-b border-[#D8C290] sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={onBack} aria-label="뒤로 가기" className="text-[#9A8155] hover:text-[#5C4A2E] transition text-lg flex-shrink-0">←</button>
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-[#3B2A16]" style={{ fontFamily: "'Gowun Batang', serif" }}>{title}</h1>
            <p className="text-xs text-[#9A8155]">사주 기반 일일 운세 분석</p>
          </div>
          {step === 'result' && (
            <div className="flex bg-[#E9DAB8] rounded-2xl p-1 gap-1 border border-[#D8C290] flex-shrink-0">
              {([{ label: '오늘', offset: 0 }, { label: '내일', offset: 1 }] as const).map(({ label, offset }) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold rounded-xl transition-all ${
                    activeOffset === offset
                      ? 'bg-[#9A6A12] text-[#0D0A1A]'
                      : 'text-[#6B5F8A] hover:text-[#5C4A2E]'
                  }`}
                >
                  {label}
                </button>
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        {step === 'form' && (
          <>
            ...
            <div className="bg-gradient-to-br from-[#FBF4E2] via-[#F3E7C8] to-[#EAD9B0] rounded-3xl p-5 shadow-xl shadow-[#000]/40 border border-[#9A6A1225]">
              <p className="text-[#9A8155] text-xs mb-2">{isToday ? '🔮 오늘' : '⏰ 내일'} · {new Date(Date.now() + activeOffset * 86400000).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}</p>
              <p className="text-2xl font-bold text-white mb-1" style={{ fontFamily: "'Gowun Batang', serif" }}>{title}</p>
              <p className="text-[#9A8155]/60 text-sm">생년월일로 나만의 {isToday ? '오늘' : '내일'} 운세를 확인하세요</p>
            </div>
            ...
            <div className="bg-[#FBF4E2] rounded-3xl border border-[#D8C290] shadow-[0_2px_20px_rgba(201,150,42,0.10)] p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-3.5 bg-gradient-to-r from-[#9A6A12] to-[#B5841C] text-[#0D0A1A] font-bold rounded-2xl shadow-lg shadow-[#9A6A1230] text-sm active:scale-[0.98] transition-all"
                >
                  {title} 확인하기 →
                </button>
              </form>
        ...
        {step === 'loading' && (
          <div className="flex flex-col items-center justify-center py-24 space-y-6">
            <div className="relative w-20 h-20">
              ...
              <div className="absolute inset-0 flex items-center justify-center"><IcTodayFortune size={32} className="text-[#9A6A12]"/></div>
            </div>
            <div className="text-center space-y-1">
              <p className="text-base font-bold text-[#3B2A16]" style={{ fontFamily: "'Gowun Batang', serif" }}>운세 분석 중...</p>
              <p className="text-sm text-[#9A8155]">사주를 풀이하고 있습니다</p>
            </div>
          </div>
        )}
        ...
        {step === 'result' && (
          <div className="space-y-4 animate-fade-in-up">
            {/* 종합 행운 게이지 */}
            <div
              className="rounded-3xl border border-[#9A6A1230] shadow-[0_2px_24px_rgba(201,150,42,0.14)] p-5 space-y-4"
              style={{ background: 'linear-gradient(160deg, #E9DAB8 0%, #150D28 55%, #0D0A1A 100%)' }}
            >
              <DayLuckGauge percent={luckPercent} star={fortune.star} />
              <div
                className="border border-[#9A6A1220] rounded-2xl p-4 flex items-center gap-3"
                style={{ background: 'linear-gradient(135deg, #201A3A 0%, #160F2C 100%)' }}
              >
                <GoldBadge size="w-9 h-9 text-lg">✨</GoldBadge>
                <div>
                  <p className="text-sm font-semibold text-[#3B2A16]">{isToday ? '오늘' : '내일'}의 전체 운세</p>
                  <p className="text-xs text-[#6E5836] mt-0.5">⭐ 종합 행운 지수: {luckPercent}%</p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div className="ml-1">
                    <p className="text-sm text-[#6E5836]">{tStem.ko}{tBranch.ko} · {tBranch.animal}</p>
                    <p className="text-xs text-[#9A8155]">{STEMS[dayStemIdx].hanja}일간 기준</p>
                  </div>
                </div>
            ...
            {/* 조언 + 주의 */}
            <div className="border border-[#9A6A1230] rounded-3xl p-4" style={{ background: 'linear-gradient(135deg, #9A6A1222 0%, #9A6A120A 100%)' }}>
              <p className="text-xs font-semibold text-[#9A6A12] mb-1.5">✨ {isToday ? '오늘의' : '내일의'} 조언</p>
            ...
            <div className="border border-red-900/40 rounded-2xl px-4 py-3 flex items-start gap-2" style={{ background: 'linear-gradient(135deg, rgba(127,29,29,0.28) 0%, rgba(127,29,29,0.10) 100%)' }}>
              ...
              <p className="text-xs text-[#6E5836] leading-relaxed">
                <span className="font-semibold text-red-400">주의 </span>{fortune.주의}
              </p>
            </div>

            {/* 분야별 */}
            <div className="rounded-3xl border border-[#9A6A1220] shadow-[0_2px_20px_rgba(201,150,42,0.10)] p-5" style={{ background: 'linear-gradient(160deg, #E9DAB8 0%, #FBF4E2 100%)' }}>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-1 h-5 bg-[#9A6A12] rounded-full" />
                <h2 className="text-base font-bold text-[#3B2A16]" style={{ fontFamily: "'Gowun Batang', serif" }}>분야별 운세</h2>
              </div>
            ...
            <div className="rounded-3xl border border-[#9A6A1220] shadow-[0_2px_20px_rgba(201,150,42,0.10)] p-5" style={{ background: 'linear-gradient(160deg, #E9DAB8 0%, #FBF4E2 100%)' }}>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-1 h-5 bg-[#9A6A12] rounded-full" />
                <h2 className="text-base font-bold text-[#3B2A16]" style={{ fontFamily: "'Gowun Batang', serif" }}>🕐 시간대별</h2>
              </div>
              <div className="space-y-2">
                {[
                  ...
                  <div key={t.label} className="flex gap-3 border border-[#9A6A1220] rounded-2xl px-4 py-3" style={{ background: 'linear-gradient(135deg, #201A3A 0%, #160F2C 100%)' }}>
                    <GoldBadge><span className="text-[#9A6A12]">{t.icon}</span></GoldBadge>
                    <div>
            ...
            <div className="rounded-3xl border border-[#9A6A1220] shadow-[0_2px_20px_rgba(201,150,42,0.10)] p-5" style={{ background: 'linear-gradient(160deg, #E9DAB8 0%, #FBF4E2 100%)' }}>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-1 h-5 bg-[#9A6A12] rounded-full" />
                <h2 className="flex items-center gap-1.5 text-base font-bold text-[#3B2A16]" style={{ fontFamily: "'Gowun Batang', serif" }}><IcCloverLucky size={18} className="text-[#9A6A12]" /> 행운 아이템</h2>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={item.label} className="border border-[#9A6A1230] rounded-2xl p-3" style={{ background: 'linear-gradient(135deg, #9A6A1222 0%, #9A6A120A 100%)' }}>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <p className="text-sm font-semibold text-[#4A3820]">{item.value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <button
              ...
              className="w-full py-3.5 text-[#3B2A16] font-semibold rounded-2xl text-sm border border-[#9A6A1240] shadow-[0_2px_20px_rgba(201,150,42,0.10)] transition active:scale-[0.98] flex items-center justify-center gap-1.5"
      ...
      <div className="text-center pb-8 text-xs text-[#A89167]">운명봄 — 양력 기준 · 사주 기반 일운</div>
    </div>
  )
}
